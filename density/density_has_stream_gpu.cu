...
__global__
void stream_compute_portion_density_kernel( uint64_cu* map, double* X, double* Y, double *W, double *H , double Gx, double Gy, int dim,
      int *my_id, int *my_count, int total_computation, int sz, int start, int dimension ){
  ...
  if( i < sz ){
    ...
    //int x = min( my_count[i]%xdim + lx, dimension - 1);
    //int y = min( my_count[i]/xdim + ly, dimension - 1);
    int x = my_count[i]%xdim + lx;
    int y = my_count[i]/xdim + ly;
...
__global__
void stream_hp_to_double_kernel( uint64_cu* map, double* map_d, int bin_num, int start ){
  auto i = blockIdx.x*blockDim.x + threadIdx.x + start;
  if( i < bin_num ){
    int N = 3;
    int K = 2;
...
int main( int argc, char* argv[] ){
  ...
  cout << pin_num << " " << Gx << "  " << Gy << '\n';
  assert( cudaSuccess == cudaMallocHost( (void**)&X, pin_num*sizeof(double) ) );
  assert( cudaSuccess == cudaMallocHost( (void**)&Y, pin_num*sizeof(double) ) );
  int num_stream = 16;
	cudaStream_t stream[num_stream];
	int my_pin_start[num_stream];
	int my_pin_end[num_stream];
	for( int i = 0 ; i < num_stream ; i ++ ){
		cudaStreamCreate(&stream[i]);
    my_pin_start[i] = i * pin_num/num_stream;
		if( i != num_stream - 1 )
      my_pin_end[i] = (i+1)*pin_num/num_stream;
		else
			my_pin_end[i] = pin_num;
	}
  ...
  cudaAlloc( hp, N*DIMENSION*DIMENSION, uint64_cu );
	...
	for( int i = 0 ; i < num_stream ; i ++ ){
    my_start[i] = i * DIMENSION*DIMENSION/num_stream;
		if( i != num_stream - 1 )
      my_end[i] = (i+1)*DIMENSION*DIMENSION/num_stream;
		else
			my_end[i] = DIMENSION*DIMENSION;
	}

	double *map;
	assert( cudaSuccess == cudaMallocHost( (void**)&map, DIMENSION*DIMENSION*sizeof(double) ) );


  int *ch = static_cast<int*>(malloc(sizeof(int)*pin_num));
	cudaMemcpy( ch, counter, sizeof(int)*pin_num, D2H );
	int my_count_start[num_stream];
	int my_count_end[num_stream];
	for( int i = 0 ; i < num_stream ; i ++ ){
    int c = 0;
		for(int j = my_pin_start[i] ; j < my_pin_end[i] ; j ++ ){
			c += ch[j];
		}
    if( i == 0 )
			my_count_start[i] = 0;
		else
			my_count_start[i] = my_count_end[i-1];
		my_count_end[i] = my_count_start[i] + c;
	}
  ...
  begin_t = std::chrono::high_resolution_clock::now();
  cudaMemset( hp, 0, N*DIMENSION*DIMENSION*sizeof(uint64_cu) ); 
  cudaDeviceSynchronize(); // cudaMemset is async

  for( int i = 0 ; i < num_stream ; i ++ ){
		auto sz = my_pin_end[i] - my_pin_start[i];
    cudaMemcpyAsync( &X_d[my_pin_start[i]], &X[my_pin_start[i]], sz*sizeof(double), H2D, stream[i]);
    cudaMemcpyAsync( &Y_d[my_pin_start[i]], &Y[my_pin_start[i]], sz*sizeof(double), H2D, stream[i]);
    dim3 DimGrid( ((my_count_end[i] - my_count_start[i])/1024+1),1,1) ;
    dim3 DimBlock(1024,1,1);
    stream_compute_portion_density_kernel<<<DimGrid,DimBlock,0,stream[i]>>>( hp, X_d, Y_d, W_d, H_d, Gx, Gy, DIMENSION,
      my_id, my_count, total_computation, my_count_end[i], my_count_start[i] , DIMENSION );
	}
  ...
  for( int i = 0 ; i < num_stream ; i ++ ){
    ...
    dim3 DimGrid(sz/1024+1,1,1);
    dim3 DimBlock(1024,1,1);
    stream_hp_to_double_kernel<<<DimGrid,DimBlock,0,stream[i]>>>( hp, map_double, sz + my_start[i], my_start[i] );
    cudaMemcpyAsync( &map[my_start[i]], &map_double[my_start[i]], (my_end[i]-my_start[i])*sizeof(double), D2H, stream[i]);
	}
	cudaDeviceSynchronize();
  ...
  std::cout << "Compute Time : "<< std::chrono::duration_cast<std::chrono::microseconds>(end_t-begin_t).count() << " micro secs\n";
